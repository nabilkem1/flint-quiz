"""Server-side timer enforcement (TEST-027 / TASK-185 / FR-015 / NFR-004).

Top-level spec-anchored timer verification. The integration-tier
detail lives at `tests/integration/test_timers.py`; this file pins
the CI-visible cases the spec lists explicitly.
"""

from __future__ import annotations

from datetime import datetime, timedelta, timezone

import pytest

from src.agent.dispatcher import Principal
from src.agent.timers import TimerVerdict, evaluate_timers
from src.agent.tools import ToolDeps, build_tools
from tests.integration._tools_fakes import RecordingEmitter, build_fake_search
from tests.integration.conftest import FakeCosmosRepository, make_session_doc

NOW = datetime(2026, 5, 17, 12, 0, 0, tzinfo=timezone.utc)
PRINCIPAL = Principal(entra_oid="user-1")


def test_evaluate_timers_returns_ok_within_budget() -> None:
    session = make_session_doc().model_copy(
        update={"started_at": NOW, "question_started_at": NOW}
    )
    assert evaluate_timers(session, now=NOW + timedelta(seconds=10)).verdict == TimerVerdict.OK


def test_quiz_expiry_supersedes_question_expiry() -> None:
    session = make_session_doc().model_copy(
        update={
            "started_at": NOW,
            "question_started_at": NOW,
            "time_limit_seconds": 600,
            "per_question_limit_seconds": 60,
        }
    )
    out = evaluate_timers(session, now=NOW + timedelta(seconds=601))
    assert out.verdict == TimerVerdict.QUIZ_EXPIRED


@pytest.mark.asyncio
async def test_submit_after_per_question_budget_records_unanswered() -> None:
    deps = ToolDeps(
        repo=FakeCosmosRepository(),
        search=build_fake_search(count=3, language="en"),  # type: ignore[arg-type]
        emitter=RecordingEmitter(),
        clock=lambda: NOW,
    )
    session = make_session_doc(n=3).model_copy(
        update={
            "shuffled_ids": [f"azure-networking-{i:03d}-en" for i in range(3)],
            # Per-quiz budget large (600s); per-question default 60s; push
            # question_started_at back 120s.
            "started_at": NOW - timedelta(seconds=120),
            "question_started_at": NOW - timedelta(seconds=120),
            "time_limit_seconds": 600,
        }
    )
    stored = await deps.repo.create_session(session)
    tools = build_tools(deps)
    result = await tools["submit_answer"](
        {
            "session_id": stored.id,
            "question_id": stored.shuffled_ids[0],
            "raw_answer": "B",
            "channel": "text",
        },
        PRINCIPAL,
    )
    assert result.ok and result.data["verdict"] == "unanswered"


@pytest.mark.asyncio
async def test_submit_after_per_quiz_budget_flips_to_scored() -> None:
    deps = ToolDeps(
        repo=FakeCosmosRepository(),
        search=build_fake_search(count=3, language="en"),  # type: ignore[arg-type]
        emitter=RecordingEmitter(),
        clock=lambda: NOW,
    )
    session = make_session_doc(n=3).model_copy(
        update={
            "shuffled_ids": [f"azure-networking-{i:03d}-en" for i in range(3)],
            "started_at": NOW - timedelta(hours=2),
            "question_started_at": NOW - timedelta(hours=2),
            "time_limit_seconds": 600,
        }
    )
    stored = await deps.repo.create_session(session)
    tools = build_tools(deps)
    result = await tools["submit_answer"](
        {
            "session_id": stored.id,
            "question_id": stored.shuffled_ids[0],
            "raw_answer": "B",
            "channel": "text",
        },
        PRINCIPAL,
    )
    assert result.ok
    assert result.data["done"] is True
    assert result.data["results"]["status"] == "Scored"
